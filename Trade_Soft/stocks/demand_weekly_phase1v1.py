...
def week_candle_classification(week_df):
	...
	week_df['Highest_of_body']=float(0)
	week_df['Height_of_lowest']= float(0)
	i = 0 
	for O,C,H,L in zip(week_df['Open'], week_df['Close'],week_df['High'],week_df['Low']):
		week_df['Height_of_lowest'].iloc[i]=float(week_df['Low'].iloc[i])
		if C==O:
			week_df['Highest_of_body'].iloc[i]=float(week_df['Close'].iloc[i])
			...
			week_df['Highest_of_body'].iloc[i]=float(week_df['Open'].iloc[i])
			if((O-C)>(H-O)+(C-L)):
		...
		i+=1
	week_df = week_df[['Open',  'High',  'Low',  'Close','Candle_Label','Highest_of_body','Height_of_lowest']]		
	return	week_df
def first_critical_point_finder(week_df):
	pointer1=-1;pointer2=-1;pointer3=-1;flag=0
	for i in range(len(week_df['Candle_Label'])-1):
		if (week_df['Candle_Label'].iloc[i]=="Green Exciting Candle" and (week_df['Candle_Label'].iloc[i+1]=="Green Boring Candle" or week_df['Candle_Label'].iloc[i+1]=="Red Boring Candle") and flag==0):
			pointer1=i
			flag=1
...
def Entry_StopLoss_Target_finder(week_df,pointer1,pointer2,pointer3):
	...
	for i in range(pointer1+1,pointer3):
		if(max_height_of_body<week_df['Highest_of_body'].iloc[i]):
			max_height_of_body=week_df['Highest_of_body'].iloc[i]
	...
	for i in range(pointer1,pointer3):
		if(least_height_of_lowests>week_df['Height_of_lowest'].iloc[i]):
			least_height_of_lowests=week_df['Height_of_lowest'].iloc[i]
...
def Param1(week_df_init,pp1,Entry):
	...
	for i in range(0,pp1):
		if(Low>week_df_init['Height_of_lowest'].iloc[i]):
			Low=week_df_init['Height_of_lowest'].iloc[i]
	if(Entry<Low):
		score+=1
	else:
		score+=0
...
def best_critical_point_finder(week_df,file_name):
	...
	while 1==1:
		...
		fresh_score,Low=Param1(week_df_init,pp1,Entry)
		print "Fresh_score:{:d}\n".format(fresh_score)
...
def get_week_Entry_StopLoss(day_df,time_duration,stock_code):
	...
	s1=str("Demand_Weekly_files/"+"Demand_weekly_"+stock_code+".txt")
	s2=str("Demand_Weekly_files/"+"Demand_weekly_"+stock_code+".csv")
	Entry,Stop_Loss,sum=best_critical_point_finder(week_df,s1)
	#write_to_csv(week_df,s2,count)
	return Entry,Stop_Loss,sum
def demand_week_execute(stock_code,time_duration):
	s="NSE/"
