...
from demand_phase1v3 import df_initialization,candle_classification,write_to_csv,first_critical_point_finder,Entry_StopLoss_Target_finder
...
def best_critical_point_finder(df,file_name,weekly_stoploss,monthly_stoploss):
	...
	while 1==1:
		...
		score=0
		fresh_score,Low=Param1(df_init,pp1,Entry)
		Gap_up_score=Param3(df,p1,p2,p3)
		time_spend_score=Param4(p1,p2)
		High_score,High=Param5(df_init,pp1,Entry,Stop_Loss)
		...
		print "score:{:d}\n".format(score)
		target.write("score:{:d}\nFresh_score={:d}\nLow_till_that_point={:f}\ntrend_score={:d}\nGap_up_score={:d}\ntime_spend_score={:d}\nHigh_score={:d}\nHigh_till_that_point={:f}\nWeekly_stoploss:{:f}\nMonthly_stoploss:{:f}\nCurve_Score:{:f}\n".format(score,fresh_score,Low,trend_score,Gap_up_score,time_spend_score,High_score,High,weekly_stoploss,monthly_stoploss,curve_score))
		if(score>=8 and fresh_score==1):
			print "Desired Zone found"
			target.write("\nDesired Zone found\n")
			print "Final values:\nEntry={:f}\nStop Loss={:f}\nscore={:d}".format(Entry,Stop_Loss,score)
			target.write("Final values:\nEntry={:f}\nStop Loss={:f}\nscore={:d}\nFresh_score={:d}\nLow_till_that_point={:f}\ntrend_score={:d}\nGap_up_score={:d}\ntime_spend_score={:d}\nHigh_score={:d}\nHigh_till_that_point={:f}\nWeekly_stoploss:{:f}\nMonthly_stoploss:{:f}\nCurve_Score:{:f}\n".format(Entry,Stop_Loss,score,fresh_score,Low,trend_score,Gap_up_score,time_spend_score,High_score,High,weekly_stoploss,monthly_stoploss,curve_score))
...
def Param1(df_init,pp1,Entry):
	...
	for i in range(0,pp1):
		if(Low>df_init['Height_of_lowest'][i]):
			Low=df_init['Height_of_lowest'][i]
	if(Entry<Low):
		score+=1
	else:
		score+=0
...
def Param3(df,p1,p2,p3):
	score=0
	if(p1>0 and df['Candle_Label'][p1-1]=="Green Exciting Candle"):
		score+=1
	else:
		score+=0
	if(min(df['Open'][p1], df['Close'][p1])>max(df['Open'][p1+1], df['Close'][p1+1])):
		score+=1
	return score
...
def Param5(df_init,pp1,Entry,Stop_loss):
	...
	High=(float("inf"))*(-1)
	for i in range(0,pp1):
		if(High<df_init['High'][i]):
			High=df_init['High'][i]
	if(Entry + (Entry-Stop_loss)*6 <=High):
		score+=2
	elif(Entry + (Entry-Stop_loss)*4<=High):
		score+=1
	else:
		score+=0
	return score,High
...
def demand_execute_function(stock_code):
	s="NSE/"
	...
	stock_code=stock_code+"_demand"
	file_name="Demand_Daywise_files/"+stock_code+".txt"
	Desired_Entry,Desired_Stop_Loss,sum=best_critical_point_finder(df,file_name,weekly_stoploss,monthly_stoploss)
	#file_name="Demand_Daywise_files/"+stock_code+".csv"
...
if __name__ == "__main__":
	...
	Desired_Entry,Desired_Stop_Loss,monthly_entry,monthly_stoploss,weekly_entry,weekly_stoploss=demand_execute_function("ACC")
