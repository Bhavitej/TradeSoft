...
from demand_phase1v3 import df_initialization,write_to_csv
...
def month_candle_classification(month_df):
	...
	month_df['Highest_of_body']=float(0)
	month_df['Height_of_lowest']= float(0)
	i = 0 
	for O,C,H,L in zip(month_df['Open'], month_df['Close'],month_df['High'],month_df['Low']):
		month_df['Height_of_lowest'].iloc[i]=float(month_df['Low'].iloc[i])
		if C==O:
			month_df['Highest_of_body'].iloc[i]=float(month_df['Close'].iloc[i])
			...
			month_df['Highest_of_body'].iloc[i]=float(month_df['Open'].iloc[i])
			if((O-C)>(H-O)+(C-L)):
		...
		i+=1
	month_df = month_df[['Open',  'High',  'Low',  'Close','Candle_Label','Highest_of_body','Height_of_lowest']]		
	return	month_df
def first_critical_point_finder(month_df):
	...
	for i in range(len(month_df['Candle_Label'])-1):
		if (month_df['Candle_Label'].iloc[i]=="Green Exciting Candle" and (month_df['Candle_Label'].iloc[i+1]=="Green Boring Candle" or month_df['Candle_Label'].iloc[i+1]=="Red Boring Candle") and flag==0):
			pointer1=i
			flag=1
...
def Entry_StopLoss_Target_finder(month_df,pointer1,pointer2,pointer3):
	...
	for i in range(pointer1+1,pointer3):
		if(max_height_of_body<month_df['Highest_of_body'].iloc[i]):
			max_height_of_body=month_df['Highest_of_body'].iloc[i]
	...
	for i in range(pointer1,pointer3):
		if(least_height_of_lowests>month_df['Height_of_lowest'].iloc[i]):
			least_height_of_lowests=month_df['Height_of_lowest'].iloc[i]
...
def Param1(month_df_init,pp1,Entry):
	...
	for i in range(0,pp1):
		if(Low>month_df_init['Height_of_lowest'].iloc[i]):
			Low=month_df_init['Height_of_lowest'].iloc[i]
	if(Entry<Low):
		score+=1
	else:
		score+=0
...
def best_critical_point_finder(month_df,file_name):
	...
	while 1==1:
		...
		fresh_score,Low=Param1(month_df_init,pp1,Entry)
		print "Fresh_score:{:d}\n".format(fresh_score)
		target.write("Fresh_score={:d}\nLow_till_that_point={:f}\n".format(fresh_score,Low))
		if(fresh_score==1):
			print "Desired Zone found"
			target.write("\nDesired Zone found\n")
			print "Final values:\nEntry={:f}\nStop Loss={:f}\n".format(Entry,Stop_Loss)
			target.write("Final values:\nEntry={:f}\nStop Loss={:f}\nFresh_score={:d}\nLow_till_that_point={:f}\n".format(Entry,Stop_Loss,fresh_score,Low))
...
def get_month_Entry_StopLoss(day_df,time_duration,stock_code):
	...
	s1=str("Demand_Monthly_files/"+"Demand_monthly_"+stock_code+".txt")
	#s2=str("/static/Demand_Monthly_files/"+"Demand_monthly_"+stock_code+".csv")
	Entry,Stop_Loss,sum=best_critical_point_finder(month_df,s1)
	#write_to_csv(month_df,s2,count)
	return Entry,Stop_Loss,sum
def demand_month_execute(stock_code,time_duration):
	s="NSE/"
