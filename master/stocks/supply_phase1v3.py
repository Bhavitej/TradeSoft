...
def candle_classification(df,time_duration):
	...
	df=df.iloc[::-1]
	df['Candle_Label'] = '-'
	df['Least_of_body']=float(0)
	df['Height_of_highest']= float(0)
	i = 0 
	df=df[:time_duration]
	#df=df[2:time_duration+2]
	for O,C,H,L in zip(df['Open'], df['Close'],df['High'],df['Low']):
		df['Height_of_highest'][i]=float(df['High'][i])                  #low of the candle
		if C==O:
			df['Least_of_body'][i]=float(df['Open'][i])
			df['Candle_Label'][i]="Green Boring Candle"
	   
	   
		elif C>O:
			df['Least_of_body'][i]=float(df['Open'][i])			   #Highest of the body of the candle
			if ((C-O)>(H-C)+(O-L)):
			...
			df['Least_of_body'][i]=float(df['Close'][i])			    #Highest of the body of the candle
			if((O-C)>(H-O)+(C-L)):
		...
		i+=1
	df = df[['Open',  'High',  'Low',  'Close','Candle_Label','Least_of_body','Height_of_highest']]		
	return	df
...
def first_critical_point_finder(df):
	...
	for i in range(len(df['Candle_Label'])-1):
		if (df['Candle_Label'][i]=="Red Exciting Candle" and (df['Candle_Label'][i+1]=="Green Boring Candle" or df['Candle_Label'][i+1]=="Red Boring Candle") and flag==0):
			pointer1=i
			flag=1
...
def Entry_StopLoss_Target_finder(df,pointer1,pointer2,pointer3):
	...
	for i in range(pointer1+1,pointer3):
		if(least_height_of_body>df['Least_of_body'][i]):
			least_height_of_body=df['Least_of_body'][i]

	print "Entry(least_height_of_body):{:02f}".format(least_height_of_body)
	Entry=least_height_of_body

	highest_height_of_highests=0
	for i in range(pointer1,pointer3):
		if(highest_height_of_highests<df['Height_of_highest'].iloc[i]):
			highest_height_of_highests=df['Height_of_highest'].iloc[i]

	print "Stop Loss(highest_height_of_highests):{:02f}".format(highest_height_of_highests)
	Stop_Loss=highest_height_of_highests
